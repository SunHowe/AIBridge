...
- 字面量字符串、注释、配置、YAML、Prefab/Scene 文本、非 C# 文件内容或普通代码内容搜索：优先使用 `$CLI text_index search`；索引不可用、未构建或范围不适用时使用 `rg -n` 和文件读取。`asset search/find` 不替代全文内容搜索，`asset read_text` 只作为 host 文件读取不可用时的 fallback。
...
- 修改前按目标选择定位工具：Unity 已导入资源/脚本/Prefab/Scene/SO 名称用 `$CLI asset search/find --format paths`；C# 语义关系用 Code Index；内容搜索优先用 `$CLI text_index search`，不可用时用 `rg`；普通仓库文件和路径正则用 `rg --files`。
