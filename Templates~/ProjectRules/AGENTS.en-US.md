...
- Before any AIBridge feature, command, menu, Workflow, Skill, Runtime, template, or documentation change, read `Doc~/KnowledgeBaseIndex.md` under AIBridgeRootDirectory (default `Packages/cn.lys.aibridge/Doc~/KnowledgeBaseIndex.md`); when needed, also read `Doc~/README.md` and the matching topic document under the same root.
