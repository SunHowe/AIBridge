...
- `code_index` 的语义查询依赖 Unity 生成的 `.sln/.csproj`；Roslyn 无法加载时会在可行场景降级为文本搜索，并明确标记 `semantic=false`。
...
`code_index` 是 CLI-only 的只读语义查询入口，不要求打开 Rider、VS Code、Cursor 或任何 IDE 窗口。CLI 会启动当前工程本地的 `AIBridgeCodeIndex` daemon，daemon 通过 Roslyn/MSBuildWorkspace 加载 Unity 生成的 solution，并返回结构化 JSON。
